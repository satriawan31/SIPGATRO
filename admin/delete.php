...
if($type == "gardu"){
    ...
    $del = mysqli_query($link, "DELETE FROM users WHERE id='$code'");
    if($del){
        echo "<script>alert('Berhasil menghapus data');window.location='list_pengguna'</script>";
    }else{
        echo "<script>alert('Gagal menghapus data');window.location='list_pengguna'</script>";
    }
}
?>